...
def make_figures(data, core, airport, risk_low, risk_high, dispersion):
    ...
    grid = np.linspace(data.demand_c.quantile(.05), data.demand_c.quantile(.95), 100)
    ...
    ax.set(xlabel='Centered log sampled passengers', ylabel='Fare at average FE composition (USD)',
           title='Exploratory interaction: fixed route/carrier and calendar composition')
    ...
    ax.set(xlabel='Lagged seasonal adverse-weather share', ylabel='Realized cancellations (% of arrivals)',
    ...
    ax.bar(['Risk Q25', 'Risk Q75'], [x['fitted_demand_component_variance_usd2'] for x in dispersion], color=colors)
    ax.set(ylabel='Fitted demand-component variance (USD²)', title='Fixed traffic distribution; not total fare volatility')
