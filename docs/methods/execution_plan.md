...
- [x] Stage 1: verified literature CSV, coverage of all required areas, novelty
  assessment; review citations and commit.
- [x] Stage 2: source matrix and five feasibility documents; select weather and
  operational validation; document available P/D/R and impossible estimands.
- [x] Acquisition: `src/acquisition/download.py` exposes a bounded cached download
...
- [x] Cleaning: `src/cleaning/db1b.py` exposes clean_market(frame) and aggregate
...
- [x] Weather: `src/weather/risk.py` exposes prior_season_risk(daily, year, quarter);
  tests future exclusion, leap-day coverage, null handling, and exact thresholds.
- [x] Analysis: `src/analysis/pilot.py` fits declared models, checks rank/support,
...
- [x] Stage 3: run acquisition and cleaning, write dictionary/missingness/duplicate/
  matching audits, commit source code, metadata, and compact permitted aggregates.
- [x] Stage 4: run every feasible analysis; report unestimable original H1/H2 and
...
Three full raw builds were performed. After canonical round-trip CSV parsing was
introduced, the final full build matched all 19 table/figure outputs byte-for-byte.
Sixteen offline tests pass. See outputs/reports/verification.json and the execution
ledger for reviewed corrections. Original quote-time tests remain unestimable.
